...
import { JsonArray, JsonAstArray, JsonAstComment, JsonAstConstantFalse, JsonAstConstantNull, JsonAstConstantTrue, JsonAstIdentifier, JsonAstKeyValue, JsonAstMultilineComment, JsonAstNode, JsonAstNumber, JsonAstObject, JsonAstString, JsonObject, JsonValue, Position } from './interface';
 ...
 * If a `valid` string is passed, validate that the character is included in the valid string.
 * @private
 */
...
/**
 * Read the exponent part of a number.
 * The exponent part is looser for JSON than the number part.
 * `str` is the string of the number itself found so far, and start the position where the full number started.
 * Returns the node found.
 * @private
 */
function _readExpNumber( context: JsonParserContext, start: Position, str: string, comments: ( JsonAstComment | JsonAstMultilineComment )[] ): JsonAstNumber {
    let char;
    let signed = false;
 ...
 * @private
 */
function _readHexaNumber( context: JsonParserContext, isNegative: boolean, start: Position, comments: ( JsonAstComment | JsonAstMultilineComment )[] ): JsonAstNumber {
...
/**
 * Read a string from the context. Takes the comments of the string or read the blanks before the string.
 * @private
 */
 ...
 * @private
 */
function _readTrue( context: JsonParserContext, comments = _readBlanks( context ) ): JsonAstConstantTrue {
    const start = context.position;
    _token( context, 't' );
    _token( context, 'r' );
    _token( context, 'u' );
 ...
 * @private
 */
function _readFalse( context: JsonParserContext, comments = _readBlanks( context ) ): JsonAstConstantFalse {
    const start = context.position;
    _token( context, 'f' );
    _token( context, 'a' );
    _token( context, 'l' );
 ...
 * @private
 */
function _readNull( context: JsonParserContext, comments = _readBlanks( context ) ): JsonAstConstantNull {
    const start = context.position;
...
/**
 * Read an identifier from the context. An identifier is a valid JavaScript identifier, and this function is only used in Loose mode.
 * @private
 */
function _readIdentifier( context: JsonParserContext, comments = _readBlanks( context ) ): JsonAstIdentifier {
    const start = context.position;
 ...
 * @private
 */
function _readProperty( context: JsonParserContext, comments = _readBlanks( context ) ): JsonAstKeyValue {
    const start = context.position;
...
/**
 * Remove any blank character or comments (in Loose mode) from the context, returning an array of comments if any are found.
 * @private
 */
...
/**
 * Parse the JSON string and return its AST.
 * The AST may be losing data (end comments are discarded for example, and space characters are not represented in the AST), but all values will have a single node in the AST (a 1-to-1 mapping).
...
export function parseJsonAst( input: string, mode = JsonParseMode.Default ): JsonAstNode {
    ...
    if ( context.position.offset < input.length ) {
        ...
        throw new Error( `Expected end of file, got "${ i }" at ` + `${ context.position.line }:${ context.position.character }.` );
    }
...
export interface ParseJsonOptions {
    /**
     * If omitted, will only emit errors related to the content of the JSON.
     * If specified, any JSON errors will also include the path of the file that caused the error.
     */
    path?: string;
}

/**
 * Parse a JSON string into its value.
 * This discards the AST and only returns the value itself.
 *
 * If a path option is pass, it also absorbs JSON parsing errors and return a new error with the path in it.
 * Useful for showing errors when parsing from a file.
 *
 ...
 */
export function parseJson( input: string, mode = JsonParseMode.Default, options?: ParseJsonOptions ): JsonValue {
    try {
