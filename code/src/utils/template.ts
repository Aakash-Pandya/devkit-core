...
export function templateParser( sourceText: string, fileName: string ): TemplateAst {
    ...
    // Optimization that uses the fact that the end of a node is always the beginning of the next node, so we keep the positioning of the nodes in memory.
    ...
    for ( let i = 0; i < parsed.length; i += increment ) {
        ...
        if ( interpolate ) {
            ...
            children.push( { kind: 'interpolate', expression: interpolate, start, end } as TemplateAstInterpolate );
            start = end;
        }
    ...
    return { fileName, content: sourceText, children };
}

/**
 * Fastest implementation of the templating algorithm. It only add strings and does not bother with source maps.
 */
...
function templateWithSourceMap( ast: TemplateAst, options?: TemplateOptions ): string {
        ...
        switch ( node.kind ) {
            case 'content':
                ...
                code = [
                    ...
                    ...node.content.split( '\n' ).map( ( line, i, arr ) => {
                        return new SourceNode( node.start.line + i, i == 0 ? node.start.column : 0, sourceUrl, '\n    + ' + JSON.stringify( line + ( i == arr.length - 1 ? '' : '\n' ) ) );
                    } ),
            ...
            case 'interpolate':
                ...
                code = [
                    ...
                    ...node.expression.split( '\n' ).map( ( line, i, arr ) => {
                        return new SourceNode( node.start.line + i, i == 0 ? node.start.column : 0, sourceUrl, line + ( ( i == arr.length - 1 ) ? '' : '\n' ) );
                    } ),
    ...
    const code = nodes.toStringWithSourceMap( { file: sourceUrl, sourceRoot: options && options.sourceRoot || '.' } );

    // Set the source content in the source map, otherwise the sourceUrl is not enough to find the content.
 ...
 *
 * This version differs from lodash by removing support from ES6 quasi-literals, and making the code slightly simpler to follow.
 * It also does not depend on any third party, which is nice.
 *
 ...
 * @return {(input: T) => string} A function that accept an input object and returns the content of the template with the input applied.
 */
export function template<T>( content: string, options?: TemplateOptions ): ( input: T ) => string {
    ...
    // We pass a dummy module in case the module option is passed.
    // If `module: true` is passed, we need to only use the source, not the function itself.
    // Otherwise expect a module object to be passed, and we use that one.
    ...
    // Provide the compiled function's source by its `toString` method or the `source` property as a convenience for inlining compiled templates.
