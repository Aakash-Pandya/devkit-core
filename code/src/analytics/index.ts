...
export { Analytics, CustomDimensionsAndMetricsOptions, EventOptions, PageviewOptions, ScreenviewOptions, TimingOptions } from './api';
export { AnalyticsForwarderFn, AnalyticsReport, AnalyticsReportBase, AnalyticsReportEvent, AnalyticsReportKind, AnalyticsReportPageview, AnalyticsReportScreenview, AnalyticsReportTiming, AnalyticsReporter, ForwardingAnalytics } from './forwarder';
export { LoggingAnalytics } from './logging';
export { MultiAnalytics } from './multi';
export { NoopAnalytics } from './noop';

/**
 * MAKE SURE TO KEEP THIS IN SYNC WITH THE TABLE AND CONTENT IN `/docs/design/analytics.md`.
 * WE LIST THOSE DIMENSIONS ( AND MORE ).
 *
 * These cannot be in their respective schema.json file because we either change the type ( e.g. --buildEventLog is string, but we want to know the usage of it, not its value ), or some validation needs to be done ( we cannot record ng add --collection if it's not whitelisted ).
 */
...
// This table is used when generating the analytics.md file. It should match the enum above or the validate-user-analytics script will fail.
