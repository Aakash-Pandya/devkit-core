...
import { Path, basename, dirname, isAbsolute, join, normalize, relative, resolve, virtualFs } from '../../virtual-fs';
import { WorkspaceProject, WorkspaceSchema, WorkspaceTool } from './workspace-schema';
import { JsonObject, JsonParseMode, parseJson, schema } from '../../json';
...
export class Workspace {
    protected static _workspaceFileNames = [ 'angular.json', '.angular.json' ];

    private readonly _workspaceSchemaPath = normalize( require.resolve( './workspace-schema.json' ) );

    private _workspaceSchema: JsonObject | undefined;

    private _workspace: WorkspaceSchema | undefined;

    private _registry: schema.CoreSchemaRegistry;

    constructor ( private _root: Path, private _host: virtualFs.Host<{}>, registry?: schema.CoreSchemaRegistry ) {
        if ( registry ) {
    ...
    static async fromPath( host: virtualFs.Host<{}>, path: Path, registry: schema.CoreSchemaRegistry ): Promise<Workspace> {
        ...
        return new Workspace( dirname( maybePath ), host, registry ).loadWorkspaceFromHost( basename( maybePath ) ).pipe( first() ).toPromise();
    }

    /**
     * @todo // TODO: Fix this method, it was broke when I got it.
     *
     * @param {JsonObject} json
     * @returns
     * @memberof Workspace
     */
    loadWorkspaceFromJson( json: JsonObject ) {
        return this._loadWorkspaceSchema().pipe(
            concatMap( ( workspaceSchema: WorkspaceSchema ) => this.validateAgainstSchema( json, workspaceSchema ) ),
    ...
    loadWorkspaceFromHost( workspacePath: Path ) {
        return this._loadWorkspaceSchemaAsJsonObject().pipe(
            concatMap(
                () => this._loadJsonFile( join( this._root, workspacePath ) )
            ),
                concatMap(
                    ( json: JsonObject ) => this.loadWorkspaceFromJson( json )
                )
        );
    }

    /**
     * @author Shelby M. Smull - Owner - SITCS
     * New method for workspaces as the latest kit is broken.
     *
     * @private
     * @returns {Observable<JsonObject>}
     * @memberof Workspace
     */
    private _loadWorkspaceSchemaAsJsonObject(): Observable<JsonObject> {
        return this._workspaceSchema ? of( this._workspaceSchema ) : new Observable<JsonObject>();
    }
     ...
     *
     * @private
     * @returns {Observable<WorkspaceSchema>}
     * @memberof Workspace
     */
    private _loadActualWorkspaceSchema(): Observable<WorkspaceSchema> {
        return new Observable<WorkspaceSchema>( () => {
            this._workspace ?
            this._workspace :
            (
                (
                    this._loadJsonFile( this._workspaceSchemaPath ).pipe(
                        tap(
                            ( workspaceSchema ) => this._workspaceSchema = workspaceSchema
                        )
                    ) as unknown
                ) as WorkspaceSchema )
        } );
    ...
    /**
     * @author Angular Team
     * This is the faulty method, since I run strict on everything, I need return types to be defined.
     *
     * @private
     * @returns {(Observable<JsonObject> | Observable<WorkspaceSchema>)}
     * @memberof Workspace
     */
    private _loadWorkspaceSchema() {
        if ( this._workspaceSchema ) {
            ...
            return this._loadJsonFile( this._workspaceSchemaPath ).pipe( tap( ( workspaceSchema ) => this._workspaceSchema = workspaceSchema ) );
        }
    }
    ...
    listProjectNames(): string[] {
        return Object.keys( ( this._workspace as WorkspaceSchema ).projects );
    }
    ...
    private _getTool( toolName: 'cli' | 'schematics' | 'targets' ): WorkspaceTool {
        ...
        let workspaceTool = ( this._workspace as WorkspaceSchema )[ toolName ];

        // Try falling back to 'architect' if 'targets' is not there or is empty.
        if ( ( !workspaceTool || Object.keys( workspaceTool ).length === 0 ) && toolName === 'targets' && ( this._workspace as WorkspaceSchema )[ 'architect' ] ) {
            workspaceTool = ( this._workspace as WorkspaceSchema )[ 'architect' ];
        }
    ...
    private _getProjectTool( projectName: string, toolName: 'cli' | 'schematics' | 'targets' ): WorkspaceTool {
        this._assertLoaded();

        const workspaceProject = ( this._workspace as WorkspaceSchema ).projects[ projectName ];
        ...
        // Try falling back to 'architect' if 'targets' is not there or is empty.
        if ( ( !projectTool || Object.keys( projectTool ).length === 0 ) && workspaceProject[ 'architect' ] && toolName === 'targets' ) {
