...
/**
 * Get the global directory for node_modules. This is based on NPM code itself, and may be subject to change, but is relatively stable.
...
function _getGlobalNodeModules() {
    ...
    return ( process.platform !== 'win32' ) ? path.resolve( globalPrefix || '', 'lib', 'node_modules' ) : path.resolve( globalPrefix || '', 'node_modules' );
}
...
/** @deprecated since version 8. Use `require.resolve` instead. */
export function setResolveHook( hook: ( ( x: string, options: ResolveOptions ) => string | null ) | null ) {
    _resolveHook = hook;
}
 ...
 * @returns {string} Path to the index to include, or if `resolvePackageJson` option was passed, a path to that file.
...
export function resolve( packageName: string, options: ResolveOptions ): string {
    ...
    function loadAsDirectorySync( x: string ): string | null {
        ...
        if ( isFile( pkgfile ) ) {
            ...
            try {
                const body = readFileSync( pkgfile, { encoding: 'utf8' } );
