...
import { JsonAstArray, JsonAstKeyValue, JsonAstNode, JsonAstObject, JsonObject, JsonValue } from '../../json';
...
export type ChangeListener = ( op: 'add' | 'remove' | 'replace', path: string, node: JsonAstArray | JsonAstObject | JsonAstKeyValue, value?: JsonValue ) => void;

type ChangeReporter = ( path: string, parent: JsonAstArray | JsonAstKeyValue | JsonAstObject, node?: JsonAstNode, old?: JsonValue, current?: JsonValue ) => void;

function findNode( parent: JsonAstArray | JsonAstObject, p: PropertyKey ): { node?: JsonAstNode; parent: JsonAstArray | JsonAstKeyValue | JsonAstObject } {
...
export function createVirtualAstObject<T extends object = JsonObject>( root: JsonAstObject, options: { exclude?: string[]; include?: string[]; listener?: ChangeListener; base?: object; } = {} ): T {
    const reporter: ChangeReporter = ( path, parent, old, current ) => {
        if ( options.listener ) {
    ...
    return create( root, '', reporter, new Set( options.exclude ), options.include && options.include.length > 0 ? new Set( options.include ) : undefined, options.base ) as T;
}

function create( ast: JsonAstObject | JsonAstArray, path: string, reporter: ChangeReporter, excluded = new Set<PropertyKey>(), included?: Set<PropertyKey>, base?: object ) {
