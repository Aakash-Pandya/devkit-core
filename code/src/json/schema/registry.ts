...
import { JsonArray, JsonObject, JsonValue, isJsonObject, BaseException, PartiallyOrderedSet, deepCopy } from '@core/*';
import { visitJson, visitJsonSchema, getTypesOfSchema, JsonSchema, JsonPointer, JsonVisitor, PromptDefinition, PromptProvider, SchemaFormat, SchemaFormatter, SchemaRegistry, SchemaValidator, SchemaValidatorError, SchemaValidatorOptions, SchemaValidatorResult, SmartDefaultProvider } from '@json-schema/*';
...
export class CoreSchemaRegistry implements SchemaRegistry {
    ...
    private _fetch( uri: string ): Promise<JsonObject> {
        ...
        for ( const maybeHandler of this._uriHandlers ) {
            ...
            if ( handler ) {
                ...
                return from( handler ).pipe( tap( json => this._uriCache.set( uri, json ) ) ).toPromise();
            }
        }
    ...
    protected _resolver( ref: string, validate?: ajv.ValidateFunction ): { context?: ajv.ValidateFunction, schema?: JsonObject } {
        ...
        if ( typeof context == 'function' ) {
            // Context will be a function if the schema isn't loaded yet, and an actual schema if it's synchronously available.
    ...
    /**
     * Flatten the Schema, resolving and replacing all the refs.
     * Makes it into a synchronous schema that is also easier to traverse.
    ...
    flatten( schema: JsonObject ): Observable<JsonObject> {
        ...
        // Supports both synchronous and asynchronous compilation, by trying the synchronous version first, then if refs are missing this will fails.
        // We also add any refs from external fetched schemas so that those will also be used in synchronous (if available).
        let validator: Observable<ajv.ValidateFunction>;
        try {
            this._currentCompilationSchemaInfo = undefined;

            validator = of( this._ajv.compile( schema ) ).pipe( tap( () => this._currentCompilationSchemaInfo = undefined ) );
        } catch ( e ) {
            // Propagate the error.
            ...
            this._currentCompilationSchemaInfo = undefined;
            validator = from( this._ajv.compileAsync( schema ) ).pipe( tap( () => this._currentCompilationSchemaInfo = undefined ) );
        }
        ...
        return validator.pipe(
            switchMap( validate => {
                ...
                function visitor( current: JsonObject | JsonArray, pointer: JsonPointer, parentSchema?: JsonObject | JsonArray, index?: string ) {
                    if ( current && parentSchema && index && isJsonObject( current ) && current.hasOwnProperty( '$ref' ) && typeof current[ '$ref' ] == 'string' ) {
    ...
    addFormat( format: SchemaFormat ): void {
        ...
        this._ajv.addFormat( format.name, { async: format.formatter.async, validate } );
    }
        ...
        if ( !this._smartDefaultKeyword ) {
            ...
            this._ajv.addKeyword( '$default', {
                ...
                compile: ( schema, _parentSchema, it ) => {
                    ...
                    // We cheat, heavily.
                    compilationSchemInfo.smartDefaultRecord.set( JSON.stringify( ( it as any ).dataPathArr.slice( 1, ( it as any ).dataLevel + 1 ) as string[] ), schema );
            ...
            compile: ( schema, parentSchema, it ) => {
                ...
                if ( !type ) {
                    if ( propertyTypes.size === 1 && propertyTypes.has( 'boolean' ) ) {
                        ...
                        type = 'list';
                    } else if ( propertyTypes.size === 1 && propertyTypes.has( 'array' ) && ( parentSchema as JsonObject ).items && Array.isArray( ( ( parentSchema as JsonObject ).items as JsonObject ).enum ) ) {
                        type = 'list';
                    } else {
                        type = 'input';
                    }
                ...
                if ( type === 'list' ) {
                    ...
                    const enumValues = multiselect ? ( parentSchema as JsonObject ).items && ( ( parentSchema as JsonObject ).items as JsonObject ).enum : ( parentSchema as JsonObject ).enum;
                ...
                const definition: PromptDefinition = {
                    ...
                    items,
                    multiselect,
                    default: typeof ( parentSchema as JsonObject ).default == 'object' && ( parentSchema as JsonObject ).default !== null && !Array.isArray( ( parentSchema as JsonObject ).default ) ? undefined : ( parentSchema as JsonObject ).default as string[], async validator( data: JsonValue ) { try { return await it.self.validate( parentSchema, data ); } catch { return false; } }
                };

                compilationSchemInfo.promptDefinitions.push( definition );

                return function ( this: { promptFieldsWithValue: Set<string> } ) {
                    // If 'this' is undefined in the call, then it defaults to the global 'this'.
