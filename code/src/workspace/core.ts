...
/**
 * Reads and constructs a `WorkspaceDefinition`.  If the function is provided with a path to a directory instead of a file, a search of the directory's files will commence to attempt to locate a known workspace file.  Currently the following are considered known workspace files:
 * - `angular.json`
 * - `.angular.json`
 *
 ...
 * @param format An optional `WorkspaceFormat` value. Used if the path specifies a non-standard file name that would prevent automatically discovering the format.
 *
 *
 * @return An `Promise` of the read result object with the `WorkspaceDefinition` contained within the `workspace` property.
 */
export async function readWorkspace( path: string, host: WorkspaceHost, format?: WorkspaceFormat ): Promise<{ workspace: WorkspaceDefinition }> { // return type will eventually have a `diagnostics` property as well
 ...
 * If the `WorkspaceDefinition` was created via the `readWorkspace` function, metadata will be used to determine the path and format of the Workspace.
 * In all other cases, the `path` and `format` options must be specified as they would be otherwise unknown.
 *
 ...
 * @param path The path to a file location for the output. Required if `readWorkspace` was not used to create the `WorkspaceDefinition`.  Optional otherwise; will override the `WorkspaceDefinition` metadata if provided.
 * @param format The `WorkspaceFormat` to use for output. Required if `readWorkspace` was not used to create the `WorkspaceDefinition`.  Optional otherwise; will override the `WorkspaceDefinition` metadata if provided.
 *
 * @return An `Promise` of type `void`.
 */
export async function writeWorkspace( workspace: WorkspaceDefinition, host: WorkspaceHost, path?: string, format?: WorkspaceFormat ): Promise<void> {
    if ( format === undefined ) {
